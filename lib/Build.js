...
class Build {
  ...
  getProgress () {
    const completed = this.totalActions - this.actions.length
    return {
      completed,
      total: this.totalActions,
      progress: this.totalActions > 0 ? completed / this.totalActions : 1
    }
  }
  ...
  getNeededItems () {
    ...
    for (const action of this.actions) {
      if (action.type === 'place') {
        const item = this.getItemForState(action.state)
        if (needed[item.id]) {
          needed[item.id].count++
        } else {
          needed[item.id] = { id: item.id, metadata: item.metadata, count: 1 }
        }
      }
    }
